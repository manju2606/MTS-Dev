...
def _confidence_and_explanation(c: dict, adx: float, macd_bullish: bool) -> tuple[float, str]:
    """General-purpose 0.0-1.0 confidence score. Weights sum to 1.0 across
    RSI zone / ADX / volume / MACD / trend -- none of them hard-reject."""
    rsi = c["rsi"]
    volume_ratio = c["volume_ratio"]

    if 45 <= rsi <= 70:
        rsi_score, rsi_note = 0.30, f"RSI {rsi:.0f} in a healthy bullish zone"
    elif 30 <= rsi < 45 or 70 < rsi <= 80:
        rsi_score, rsi_note = 0.15, f"RSI {rsi:.0f} — moderate"
    else:
        rsi_score, rsi_note = 0.05, f"RSI {rsi:.0f} — extended"

    if adx > 25:
        adx_score, adx_note = 0.25, f"ADX {adx:.0f} — strong trend"
    elif adx > 20:
        adx_score, adx_note = 0.15, f"ADX {adx:.0f} — rising trend"
    else:
        adx_score, adx_note = 0.05, f"ADX {adx:.0f} — weak/no trend"
    ...
    macd_score = 0.15 if macd_bullish else 0.0
    trend_score = 0.05 if c["current"] > c["sma20"] > c["sma50"] else 0.0
...
def _size_entry_sl_target(c: dict) -> tuple[float, float, float, float]:
    ...
    target = round(entry * (1 + 1.5 * atr_pct / 100), 2)
...
async def process_chartink_alert(
    ...
    for c in technicals:
        try:
            ...
            confidence, explanation = _confidence_and_explanation(c, adx, macd_bullish)
            entry, stop_loss, target, rr = _size_entry_sl_target(c)
