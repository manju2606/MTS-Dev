"""MTS Gold Strategy -- a dedicated multi-timeframe (1H trend / 15M setup /
5M entry) intraday scorer for MCX Gold, GoldMini, GoldTen, GoldGuinea, and
GoldPetal, distinct from the generic single-timeframe Metals-AI Pro score
(mcx_metals_ai_score_service.py) that already covers all 17 tracked metals
contracts. Sibling module to mcx_silver_strategy_service.py -- same scoring
logic, mirrored rather than shared as a generalized module, same "parallel,
self-contained module so nothing here can regress the already-stabilized
Silver feature" reasoning mcx_metals_service.py itself gives for its own
NG/Metals split.
...
indicator math, _check/_category/_reason_for from mcx_ai_score_service.py
for the same category-scoring shape the rest of MCX AI scoring already
uses, and SilverRiskConfig/mcx_silver_risk_gate.py's pure functions
directly -- despite the "Silver" name, nothing about that risk-state
config/gate logic is Silver-specific (it's just daily trade-count/loss/
consecutive-loss/session/expiry math over already-fetched data), so it's
reused as-is rather than duplicated under a Gold-specific name.
...
as the MTS Silver Strategy spec ("Do not trade during neutral conditions").

Signals are persisted through the existing McxSignalRepository (mcx_trade_signals
collection) under a synthetic contract key ("GOLD_MTS"/"GOLDMINI_MTS"/etc.)
so they never mix with the generic Metals-AI Pro signals already tracked for
plain "GOLD"/"GOLDMINI"/etc. -- same synthetic-key convention NG-AI Pro v2.0
already uses ("NG_V2") to keep separate signal populations apart in one
shared collection.
"""
...
# All 5 Gold variants MCX tracks -- broader than Silver's 2 (SILVER100/
# SILVERMICRO), per explicit product choice rather than the same "just the
# two most commonly traded" scoping Silver used.
GOLD_STRATEGY_CONTRACTS: list[str] = ["GOLD", "GOLDMINI", "GOLDTEN", "GOLDGUINEA", "GOLDPETAL"]

# contract -> synthetic signal-tracking key (see module docstring).
_SIGNAL_CONTRACT_KEY: dict[str, str] = {c: f"{c}_MTS" for c in GOLD_STRATEGY_CONTRACTS}
...
class MtsGoldParams:
    """Every numeric threshold the MTS strategy spec calls out as
    configurable. Same defaults as MtsSilverParams -- no Gold-specific
    research yet to justify different thresholds."""
...
# Valid ATR-multiplier optimization inputs -- not enforced, just the
# documented set a caller is expected to pick from.
...
def _hourly_trend(candles: list[dict], params: MtsGoldParams) -> tuple[str, list[dict]]:
    """Returns ("bullish"|"bearish"|"neutral", checks): EMA20 vs EMA50,
    price vs EMA20, RSI band."""
...
def _setup_15m(
    ...
    pdh: float | None,
    pdl: float | None,
    params: MtsGoldParams,
) -> dict:
    """VWAP alignment, EMA20/VWAP pullback, RSI, reversal candle / structure
    break, volume."""
...
def _confirm_5m(candles: list[dict], direction: str, params: MtsGoldParams) -> dict:
    """EMA9/20 cross, VWAP, RSI, volume, breakout of the prior 5M candle's
    high/low."""
    c, h, low = ind.closes(candles), ind.highs(candles), ind.lows(candles)
    vols = ind.volumes(candles)
...
def _classify_gold(score_pct: float, params: MtsGoldParams) -> str:
...
async def compute_gold_strategy_score(
    user_id: str,
    direction: str,
    contract: str = "GOLD",
    capital: float = 100_000.0,
    account_risk_pct: float = 0.5,
    params: MtsGoldParams | None = None,
) -> dict:
    """Full MTS Gold Strategy breakdown for one direction on any of
    GOLD_STRATEGY_CONTRACTS. Raises ValueError if there isn't enough candle
    history yet, same convention as compute_metal_ai_score."""
    if contract.upper() not in GOLD_STRATEGY_CONTRACTS:
        raise ValueError(
            f"MTS Gold Strategy only covers {GOLD_STRATEGY_CONTRACTS}, got '{contract}'"
        )
    params = params or MtsGoldParams()
    ...
    # Split the 15M/5M checks across the same point buckets as MTS Silver
    # Strategy: VWAP 15, EMA 10, RSI 10, pullback 10, reversal 10, volume 10,
    # PDH/PDL 10, 5M confirmation 5.
    ...
    # 1H trend is a hard gate -- a neutral/opposing 1H trend forces NO_TRADE
    # regardless of how well the lower timeframes score.
    verdict = _classify_gold(score_pct, params) if trend_matches else "NO_TRADE"
    ...
    return {
        ...
        "reasoning": build_gold_reasoning(
...
# ── Risk gate ─────────────────────────────────────────────────────────────
#
# Wraps app/domain/services/mcx_silver_risk_gate.py's pure functions (fully
# generic despite the "Silver" name -- see module docstring) with the actual
# data fetches (today's closed signals, contract expiry) they need -- kept
# separate from the gate module itself so that module stays testable
# without a database or broker session.


async def get_gold_risk_status(
    user_id: str,
    contract: str,
    ...
    check_and_log_gold_signal needs to decide whether a new signal is even
...
async def can_generate_new_signal(
    ...
    status = await get_gold_risk_status(user_id, contract, repo, cfg)
...
async def check_and_log_gold_signal(
    ...
    await send_strategy_signal_alert(
        user_id, "MTS Gold Strategy", contract.upper(), contract.upper(), score,
        "/mcx/metals/goldguinea" if contract.upper() == "GOLDGUINEA" else "/mcx/metals",
    )
    return True


async def resolve_open_gold_signals(
...
def _serialize_gold_signal(doc: dict) -> dict:
    return {
...
async def list_gold_signals_with_accuracy(
    ...
    before querying, so callers can pass any GOLD_STRATEGY_CONTRACTS value
    the same way they do for every other MCX endpoint."""
    signal_key = _SIGNAL_CONTRACT_KEY[contract.upper()]
    ...
    return {
        "contract": contract.upper(),
        "signals": [_serialize_gold_signal(s) for s in signals],
...
# which can't represent this strategy's SELL signals, and MCX Gold is a
...
async def run_gold_strategy_backtest(
