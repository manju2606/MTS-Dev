...
router = APIRouter(prefix="/kotegawa", tags=["kotegawa"])
...
async def get_latest(_: CurrentUser) -> dict:
    """Return the most recent Kotegawa scan, or 404 if none exists yet."""
    ...
    if doc is None:
        raise HTTPException(status_code=404, detail="No Kotegawa scan found. Run a scan first.")
...
async def get_history(
    ...
    """Return metadata for recent Kotegawa scans (no picks detail), most recent first."""
    from app.infra.db.repositories.kotegawa_repo import KotegawaRepository
...
async def get_scan_by_date(date_str: str, _: CurrentUser) -> dict:
    """Return the full Kotegawa scan document for a specific date (YYYY-MM-DD)."""
    from app.infra.db.repositories.kotegawa_repo import KotegawaRepository

    repo = KotegawaRepository()
    doc = await repo.get_scan_by_date(date_str)
    if doc is None:
        raise HTTPException(status_code=404, detail=f"No Kotegawa scan found for date {date_str}")
    await _attach_pnl_to_picks(doc.get("picks", []))
    return doc
...
async def trigger_scan(_: CurrentUser) -> dict:
    """Manually trigger a Kotegawa scan (admin only). Runs the full scan pipeline."""
    import dataclasses

    from app.services.kotegawa_service import run_and_save_kotegawa

    scan = await run_and_save_kotegawa()
...
async def get_performance(_: CurrentUser) -> dict:
    """Return accuracy statistics for resolved Kotegawa picks (hit rate, avg return)."""
    from app.infra.db.repositories.kotegawa_repo import KotegawaRepository
