...
# ── Portfolio Assistant — Period Summary (Day/Week/Month) ────────────────────
...
async def assistant_summary(
    current_user: CurrentUser,
    portfolio_id: str = QParam(default="default"),
    period: str = QParam(default="week"),
    date: str | None = QParam(
        default=None,
        description="YYYY-MM-DD, only used when period=day. Past dates are read "
        "from the stored EOD snapshot; today is always computed live.",
    ),
) -> dict:
    """How the portfolio performed over day/week/month: winners, losers, sector
    moves vs Nifty, and rule-based suggestions on what to review."""
    ...
    today_str = dt.now(IST).strftime("%Y-%m-%d")

    if period == "day" and date and date != today_str:
        from app.infra.db.repositories.portfolio_summary_repo import PortfolioSummaryRepository

        stored = await PortfolioSummaryRepository().get_snapshot(user_id, portfolio_id, date)
        if stored:
            return stored
        return {"period": "day", "has_data": False, "date": date}

    return await compute_portfolio_summary(user_id, portfolio_id, period)


# ── Portfolio Assistant — OHLC ────────────────────────────────────────────────


@router.get("/assistant/ohlc")
async def assistant_ohlc(
    current_user: CurrentUser,
    portfolio_id: str = QParam(default="default"),
) -> dict:
    """Per-holding OHLC, daily/weekly/monthly change, and 52-week high/low."""
    from app.services.portfolio_ohlc_service import compute_portfolio_ohlc

    return await compute_portfolio_ohlc(str(current_user.id), portfolio_id)
...
async def assistant_tax(
    ...
    return {
        ...
        "summary": {
            ...
            "note": (
                "Tax rates: STCG 20%, LTCG 12.5% (Budget 2024). LTCG exempt up to ₹1.25L per FY."
            ),
        },
    }
