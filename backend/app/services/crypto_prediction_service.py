...
fresh per request, which is cheap. What *is* cached is the OHLC candle
data this reads (crypto_service.get_ohlc, 5-min TTL), proactively kept
warm by the scheduler's crypto_prediction_prewarm job (every 4 min) --
without that, a cold cache on get_ranked_predictions() would mean the
first real request pays CoinGecko's rate-limited cold-start cost (see
crypto_service.py's own docstring on why that's real, not theoretical).
"""
...
async def get_prediction(coin: str, period: str = "30m") -> dict:
    candles = await get_ohlc(coin, period)
...
async def get_ranked_predictions() -> dict:
    ...
    at each OHLC_PERIODS timeframe, ranked by 24H change descending (same
    ...
    async def _row(code: str) -> dict:
        preds = await asyncio.gather(*[_nearest_predicted(code, p) for p in OHLC_PERIODS])
        ...
        return {
            ...
            "predicted": dict(zip(OHLC_PERIODS.keys(), preds, strict=True)),
        }
