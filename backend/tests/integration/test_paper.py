...
async def test_place_trade_buy_stop_above_entry(client: AsyncClient, auth: dict):
    resp = await client.post(
        ...
        json={
            **VALID_BUY,
            "stop_loss": 1050.0,  # above ₹1000 — invalid for BUY
        },
...
async def test_place_trade_buy_target_below_entry(client: AsyncClient, auth: dict):
    resp = await client.post(
        ...
        json={
            **VALID_BUY,
            "target": 950.0,  # below ₹1000 — invalid for BUY
        },
        headers=_headers(auth["token"]),
    )
    assert resp.status_code == 422
...
async def test_place_trade_sell_stop_below_entry(client: AsyncClient, auth: dict):
    resp = await client.post(
        ...
        json={
            **VALID_SELL,
            "stop_loss": 950.0,  # below ₹1000 — invalid for SELL
        },
        headers=_headers(auth["token"]),
    )
    assert resp.status_code == 422
...
async def test_list_trades_filter_by_status(client: AsyncClient, auth: dict):
    place = await client.post(BASE + "/trades", json=VALID_BUY, headers=_headers(auth["token"]))
    trade_id = place.json()["id"]
    await client.post(BASE + f"/trades/{trade_id}/close", headers=_headers(auth["token"]))

    open_resp = await client.get(BASE + "/trades?status=open", headers=_headers(auth["token"]))
    closed_resp = await client.get(BASE + "/trades?status=closed", headers=_headers(auth["token"]))
...
async def test_close_trade(client: AsyncClient, auth: dict):
    ...
    resp = await client.post(BASE + f"/trades/{trade_id}/close", headers=_headers(auth["token"]))
    assert resp.status_code == 200
    body = resp.json()
...
async def test_close_nonexistent_trade(client: AsyncClient, auth: dict):
    resp = await client.post(BASE + f"/trades/{uuid4()}/close", headers=_headers(auth["token"]))
    assert resp.status_code == 404
