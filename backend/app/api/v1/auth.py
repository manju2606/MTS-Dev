...
async def _seed_default_watchlists(db, user_id: UUID) -> None:
    ...
    _insert_watchlist = text(
        "INSERT INTO watchlists (id, user_id, name, created_at) "
        "VALUES (:id, :uid, :name, NOW()) "
        "ON CONFLICT ON CONSTRAINT uq_watchlist_user_name DO NOTHING"
    )
    try:
        uid = str(user_id)
        # My Watchlist
        await db.execute(
            _insert_watchlist,
            {"id": str(uuid4()), "uid": uid, "name": "My Watchlist"},
        )
        # Nifty 50
        wl_id = str(uuid4())
        await db.execute(
            _insert_watchlist,
            {"id": wl_id, "uid": uid, "name": "Nifty 50"},
        )
        ...
        for sym in _NIFTY_50:
            await db.execute(
                text(
                    ...
                    "ON CONFLICT ON CONSTRAINT uq_watchlist_item_symbol DO NOTHING"
                ),
        ...
        await db.execute(
            _insert_watchlist,
            {"id": str(uuid4()), "uid": uid, "name": "Stock of the Day"},
        )
        await db.commit()
