"""Kotegawa Intraday API routes. Mirrors kotegawa.py's shape."""
...
router = APIRouter(prefix="/kotegawa-intraday", tags=["kotegawa"])
...
async def get_latest(_: CurrentUser) -> dict:
    """Return today's Kotegawa Intraday scan doc (accumulates across the
    day's repeated runs against NIFTY_100), or 404 if none exists yet."""
    ...
    if doc is None:
        raise HTTPException(
            status_code=404, detail="No Kotegawa Intraday scan found. Run a scan first."
        )
...
async def get_scan_by_date(date_str: str, _: CurrentUser) -> dict:
    ...
    if doc is None:
        raise HTTPException(
            status_code=404, detail=f"No Kotegawa Intraday scan found for date {date_str}"
        )
    await _attach_pnl_to_picks(doc.get("picks", []))
    return doc
...
async def trigger_scan(_: CurrentUser) -> dict:
    """Manually trigger one Kotegawa Intraday scan pass (admin only)."""
    import dataclasses

    from app.services.kotegawa_intraday_service import run_and_save_kotegawa_intraday

    scan = await run_and_save_kotegawa_intraday()
