...
export default function ResearchView() {
  ...
  async function handleAddWatchlist(symbol: string) {
    ...
    } catch { /* already in watchlist */ setAddedSymbols(prev => new Set([...prev, symbol])) }
  }
        ...
        <div className="mb-6 flex flex-wrap items-center gap-3">
          ...
          {results.length > 0 && (
            <span className="text-xs text-zinc-400">
              {results.length} stocks · <span className="text-emerald-600 dark:text-emerald-400">{buyCount} BUY</span> · <span className="text-red-500 dark:text-red-400">{sellCount} SELL</span>
            </span>
          )}
        </div>
            ...
            <table className="w-full text-xs">
              <thead>
                <tr className="border-b border-zinc-100 dark:border-zinc-800">
                  ...
                  <th className="px-4 py-3 text-left font-medium text-zinc-400">Symbol</th>
                  <th className="px-4 py-3 text-left font-medium text-zinc-400">Signal</th>
                  <th className="px-4 py-3 text-right font-medium text-zinc-400">Price</th>
                  <th className="px-4 py-3 text-right font-medium text-zinc-400">Chg%</th>
                  <th className="px-4 py-3 text-left font-medium text-zinc-400">RSI</th>
                  <th className="px-4 py-3 text-left font-medium text-zinc-400">Momentum</th>
                  ...
                  <th className="px-4 py-3 text-left font-medium text-zinc-400">Rationale</th>
              ...
              <tbody>
                {results.map((r, i) => {
                  ...
                  const scoreColor = r.combined_score >= 65 ? 'text-emerald-600 dark:text-emerald-400'
                    : r.combined_score <= 38 ? 'text-red-500 dark:text-red-400'
                  ...
                  return (
                    <tr key={r.symbol} className="border-b border-zinc-50 hover:bg-zinc-50/60 dark:border-zinc-800/50 dark:hover:bg-zinc-800/30">
                      ...
                      <td className="px-4 py-3">
                        ...
                        <p className="text-[10px] text-zinc-400 truncate max-w-[80px]">{r.name}</p>
                      </td>
                      ...
                      <td className={`px-4 py-3 text-right font-mono font-semibold ${r.change_pct >= 0 ? 'text-emerald-600 dark:text-emerald-400' : 'text-red-500 dark:text-red-400'}`}>
                        {r.change_pct >= 0 ? '+' : ''}{r.change_pct.toFixed(2)}%
                      </td>
                      <td className="px-4 py-3">
                        <span className={`font-mono ${r.rsi > 65 ? 'text-red-500' : r.rsi < 35 ? 'text-emerald-600' : 'text-zinc-600 dark:text-zinc-300'}`}>
                          {r.rsi.toFixed(0)}
                        </span>
                      </td>
                      <td className="px-4 py-3">
                      ...
                      <td className="px-4 py-3 max-w-[180px]">
                        <div className="flex flex-wrap gap-1">
                          {r.rationale.map((tag, ti) => (
                            <span key={ti} className="rounded bg-zinc-100 px-1.5 py-0.5 text-[10px] text-zinc-500 dark:bg-zinc-800 dark:text-zinc-400">
                              {tag}
                            </span>
                          ))}
                        </div>
                      ...
                      <td className="px-4 py-3">
                        <div className="flex items-center gap-2">
                          <button
                            onClick={() => handleAddWatchlist(r.symbol)}
                            disabled={isAdded}
                          ...
                          <Link
                            href={`/ai?symbol=${encodeURIComponent(r.symbol)}`}
