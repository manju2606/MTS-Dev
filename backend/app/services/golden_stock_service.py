...
async def _create_intraday_watchlist(scan: GoldenStockScan) -> None:
    """Create an Intraday watchlist in PostgreSQL for all admin users (top 1 pick only)."""
    if not scan.picks:
        return
    try:
        ...
        wl_name = f"Intraday {datetime.now(IST).strftime('%Y-%m-%d')}"
        ...
        log.info("golden_stock.watchlist.created", name=wl_name, top_pick=top_pick.symbol)
    except Exception as exc:
