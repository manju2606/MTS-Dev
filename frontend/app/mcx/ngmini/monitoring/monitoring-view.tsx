...
import { getNgQuote, getGasStrategyScore, getGasStrategyRiskStatus } from '@/lib/api'
import type { NgQuote, GasStrategyScore, GasRiskStatus } from '@/lib/api'
import { cls, pnlColor } from '../../metals/metals-view'
import { GasRiskStatusCard, NgMiniSubNav } from '../ngmini-view'
  ...
  return (
    <div className="rounded-xl border border-zinc-200 bg-white p-5 dark:border-zinc-800 dark:bg-zinc-900">
      <div className="mb-2 flex items-center justify-between">
        ...
        {score && (
          <span className={cls('rounded-full px-2.5 py-0.5 text-[10px] font-bold', GAS_VERDICT_STYLE[score.verdict])}>
            {score.signal_label}
          </span>
        )}
      </div>
...
export default function NgMiniMonitoringView() {
  const [quote, setQuote] = useState<NgQuote | null>(null)
  const [buyScore, setBuyScore] = useState<GasStrategyScore | null>(null)
  const [sellScore, setSellScore] = useState<GasStrategyScore | null>(null)
  ...
  const refresh = useCallback(async () => {
    ...
    const [q, buy, sell, risk] = await Promise.allSettled([
      withTimeout(getNgQuote(t, 'NGMINI'), CALL_TIMEOUT_MS, 'Quote'),
      withTimeout(getGasStrategyScore(t, 'BUY', 'NGMINI'), CALL_TIMEOUT_MS, 'BUY score'),
      withTimeout(getGasStrategyScore(t, 'SELL', 'NGMINI'), CALL_TIMEOUT_MS, 'SELL score'),
      withTimeout(getGasStrategyRiskStatus(t, 'NGMINI'), CALL_TIMEOUT_MS, 'Risk status'),
    ])
  ...
  return (
    <div className="min-h-screen bg-zinc-50 dark:bg-zinc-950">
      ...
      <div className="mx-auto max-w-4xl px-4 py-8">
        <h1 className="text-2xl font-bold text-zinc-900 dark:text-zinc-50">NG Mini — Monitoring</h1>
        ...
        <div className="mt-6 space-y-6">
          <NgMiniSubNav active="Monitoring" />
