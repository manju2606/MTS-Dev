...
async def _run_mcx_silver_strategy_check() -> None:
    """MTS Silver Strategy's own signal-check job -- same per-user,
    per-contract loop as _run_mcx_metals_signal_check, scoped to just
    SILVER100/SILVERMICRO (see mcx_silver_strategy_service.SILVER_STRATEGY_CONTRACTS)
    and using the strategy's own two-stage (target_1 partial exit, then
    target_2/breakeven) resolver instead of the single-target one."""
    try:
        from app.infra.brokers import session_store
        from app.infra.db.repositories.mcx_signal_repo import McxSignalRepository
        from app.services.mcx_silver_strategy_service import (
            SILVER_STRATEGY_CONTRACTS,
            can_generate_new_signal,
            check_and_log_silver_signal,
            compute_silver_strategy_score,
            resolve_open_silver_signals,
        )
        ...
        for user_id in user_ids:
            for contract in SILVER_STRATEGY_CONTRACTS:
                try:
                    ...
                    changed += await resolve_open_silver_signals(user_id, contract, repo)
                    ...
                    for direction in ("BUY", "SELL"):
                        score = await compute_silver_strategy_score(
                            user_id, direction, contract, 100_000.0
                        )
                        if await check_and_log_silver_signal(
                            user_id, contract, direction, score, repo
                        ):
                            logged += 1
                except Exception as exc:
                    log.warning(
                        "scheduler.mcx_silver_strategy.contract_error",
                        user_id=user_id,
                        contract=contract,
                        error=str(exc),
                    )
                await asyncio.sleep(0)
        log.info(
            "scheduler.mcx_silver_strategy.done",
...
async def _run_mcx_ng_strategy_check() -> None:
    """MTS Natural Gas Strategy's own signal-check job -- sibling to
    _run_mcx_gold_strategy_check/_run_mcx_silver_strategy_check, scoped to
    NG_STRATEGY_CONTRACTS (just NGMINI). Previously missing entirely --
    NG Mini's strategy score was only ever computed live on page view, so
    signals never got logged or alerted on in the background like Gold and
    Silver's do."""
    try:
        from app.infra.brokers import session_store
        from app.infra.db.repositories.mcx_signal_repo import McxSignalRepository
        from app.services.mcx_ng_strategy_service import (
            NG_STRATEGY_CONTRACTS,
            can_generate_new_signal,
            check_and_log_ng_signal,
            compute_ng_strategy_score,
            resolve_open_ng_signals,
        )
        ...
        logged, changed, blocked = 0, 0, 0
        for user_id in user_ids:
            for contract in NG_STRATEGY_CONTRACTS:
                try:
                    changed += await resolve_open_ng_signals(user_id, contract, repo)
                    ...
                    for direction in ("BUY", "SELL"):
                        score = await compute_ng_strategy_score(
                            user_id, direction, contract, 100_000.0
                        )
                        if await check_and_log_ng_signal(
                            user_id, contract, direction, score, repo
                        ):
                            logged += 1
                except Exception as exc:
                    log.warning(
                        "scheduler.mcx_ng_strategy.contract_error",
                        user_id=user_id,
                        contract=contract,
                        error=str(exc),
                    )
                await asyncio.sleep(0)
        log.info(
            "scheduler.mcx_ng_strategy.done",
            users=len(user_ids), logged=logged, changed=changed, blocked=blocked,
        )
    except Exception as exc:
        log.error("scheduler.mcx_ng_strategy.error", error=str(exc))
...
def start_scheduler() -> None:
    ...
    _scheduler.add_job(
        _run_mcx_silver_strategy_check,
        CronTrigger(
            day_of_week="mon-fri", hour="9-23", minute="*/5", second=35,
            timezone="Asia/Kolkata",
        ),
    ...
    _scheduler.add_job(
        _run_mcx_ng_strategy_check,
        CronTrigger(
            day_of_week="mon-fri", hour="9-23", minute="*/5", second=50,
            timezone="Asia/Kolkata",
        ),
        id="mcx_ng_strategy_check",
        name="MCX NG Strategy — MTF Signal Logging + Resolution",
        max_instances=1,
        misfire_grace_time=180,
    )
