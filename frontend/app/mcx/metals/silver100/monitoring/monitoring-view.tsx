...
import { getMetalQuote, getSilverStrategyScore, getSilverStrategyRiskStatus } from '@/lib/api'
import type { NgQuote, SilverStrategyScore, SilverRiskStatus } from '@/lib/api'
import { SilverRiskStatusCard, SILVER_VERDICT_STYLE, cls, pnlColor } from '../../metals-view'
...
function DirectionCard({ direction, score, loading }: {
  direction: 'BUY' | 'SELL'
  score: SilverStrategyScore | null
  loading: boolean
}) {
  return (
    <div className="rounded-xl border border-zinc-200 bg-white p-5 dark:border-zinc-800 dark:bg-zinc-900">
      <div className="mb-2 flex items-center justify-between">
        ...
        {score && (
          <span className={cls('rounded-full px-2.5 py-0.5 text-[10px] font-bold', SILVER_VERDICT_STYLE[score.verdict])}>
            {score.signal_label}
          </span>
        )}
      </div>
...
export default function Silver100MonitoringView() {
  const [quote, setQuote] = useState<NgQuote | null>(null)
  const [buyScore, setBuyScore] = useState<SilverStrategyScore | null>(null)
  const [sellScore, setSellScore] = useState<SilverStrategyScore | null>(null)
  ...
  const refresh = useCallback(async () => {
    ...
    const [q, buy, sell, risk] = await Promise.allSettled([
      withTimeout(getMetalQuote(t, 'SILVER100'), CALL_TIMEOUT_MS, 'Quote'),
      withTimeout(getSilverStrategyScore(t, 'BUY', 'SILVER100'), CALL_TIMEOUT_MS, 'BUY score'),
      withTimeout(getSilverStrategyScore(t, 'SELL', 'SILVER100'), CALL_TIMEOUT_MS, 'SELL score'),
      withTimeout(getSilverStrategyRiskStatus(t, 'SILVER100'), CALL_TIMEOUT_MS, 'Risk status'),
    ])
  ...
  return (
    <div className="min-h-screen bg-zinc-50 dark:bg-zinc-950">
      ...
      <div className="mx-auto max-w-4xl px-4 py-8">
        <h1 className="text-2xl font-bold text-zinc-900 dark:text-zinc-50">Silver100 — Monitoring</h1>
        ...
        <div className="mt-6 space-y-6">
          <Silver100SubNav active="Monitoring" />
